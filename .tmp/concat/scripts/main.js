  ...
  function pathShrink(d, i, pathClass, transitionTime) {
    linksLayer.select(pathClass + ":nth-child(" + (i + 1) + ")")
      ...
      .style("stroke", function(d) {
        return rdBuScale(d.male / d.units)
      })
  ...
  d3.json("data/mobility_transfers.json", function(data) {
  ...
  function drawSliderBar(data) {
    ...
    d3.selectAll('line.tick')
      .on('click', function(d) {
        ...
        hideLinks(200, 1, 3); // quick transition time
        hideMarkers(200, 1, 3); // quick transition time
        setTimeout(function() {
          d3.json("data/mobility_transfers_year" + year + ".json", function(data) {
            showTransfers(data, 1000); // quick transition time
          });
        }, 500);

        d3.selectAll('g.x.axis text')
          .classed("text on", false);
        d3.selectAll('line.tick')
          .classed("on", false);
        d3.select(this)
          .classed("on", true);
    ...
    d3.selectAll('g.x.axis text')
      .on('click', function(d) {
        ...
        setTimeout(function() {
          d3.json("data/mobility_transfers_year" + year + ".json", function(data) {
            showAllTransfers(data, 500); // quick transition time
          });
        }, 1000);
    ...
    $(function() {
      $("input[type=submit]")
        ...
        .click(function() {
          ...
          if (buttonValue == "Animate") {
            console.log('Animating......');
            var years = [];
            d3.selectAll('g.x.axis text').each(function(d) {
              years.push(d.getFullYear().toString().substring(2, 4));
            });
            console.log(years);
            (function myLoop(i) {
              setTimeout(function() {
                var year = years[years.length - i]; //  your code here   
                console.log(year);
                hideLinks(200, 1, 3); // quick transition time
                hideMarkers(200, 1, 3); // quick transition time
                d3.selectAll('g.x.axis text')
                ...
                var j = years.length - parseInt(i);
                console.log(j);
                console.log(d3.selectAll('g.x.axis text'));
                console.log(d3.selectAll('line.tick'));

                // $( "g.x.axis text" )[j].addClass( 'text on' );
                // $( "line.tick" )[0][j].addClass( 'on' );
                $( "g.x.axis text" )[j].classList.add('text')
                $( "g.x.axis text" )[j].classList.add('on')
                $( "line.tick" )[j].classList.add('on')

                // d3.selectAll('g.x.axis text:nth-child(' + (j+1) + ')')
                //   .classed("text on", true);
                // d3.selectAll('line.tick:nth-child(' + (j+1) + ')')
                //   .classed("on", true);
                setTimeout(function() {
                  d3.json("data/mobility_transfers_year" + year + ".json", function(data) {
                    showAllTransfers(data, 500); // quick transition time
    ...
    data.forEach(function(d) {
      if (d.destinations) {
        ...
        for (var j = 0; j < d.destinations.length; j++) {
          tableData.rows.push(
            [d.name, d.destinations[j].name, d.destinations[j].units.toString(), Math.round((d.destinations[j].male / d.destinations[j].units) * 100)])
        }
      }
    })
    ...
    var outerRadiusScale = {
      ...
      function makeSourceLinks(d, i) {
        ...
        for (var i = 0; i < d.sources.length; i++) {
          tableData.rows.push(
            [d.sources[i].name, d.name, d.sources[i].units.toString(), Math.round((d.sources[i].male / d.sources[i].units) * 100)])
        }
        d3.select('div#table')
          .datum(tableData)
          .call(tableFunc)
